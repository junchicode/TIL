    ...
    if money > 0:
        ...
        if budget < mincost:
            ...
            if len(abletobuy) == 0:
                ...
                pass
            while True:
                whichonetobuy = int(input("구매하실 메뉴의 번호를 입력하세요. : "))
                whichonetobuy = whichonetobuy - 1
                if whichonetobuy in abletobuy:
                    print("")
                    print(f"{menus[whichonetobuy]}를 구매하셨습니다.")
                    change = budget - costs[whichonetobuy]
                    print(f"거스름돈은 {change}원입니다. 감사합니다.")
                    break
                else:
